  ...
  showDetailProduct(detailProduct) {
    ...
    if (detailProduct.length == 0) {
      document
        ...
        .setAttribute("content", "Ceci est une page d'erreur.");
      document
      ...
      document
        ...
        .setAttribute(
          "content",
          "Découvrez les particularités de " +
            detailProduct.name +
            ", nounours d'exception."
        );
      document
        .querySelector('meta[property="og:description"]')
        .setAttribute(
      ...
      document.getElementById("container").innerHTML = /*html*/ `
                ...
                <section class="detailProduct">
                    <div>
                        <img src="${detailProduct.imageUrl}" alt="Nounours ${
        detailProduct.name
      }"/>
                    </div>
                    <div>
    ...
    for (let productBought of productsBought) {
      document.getElementById("listProductBought").innerHTML += /*html*/ `
            <article class="basket">
                <div><img src= "${productBought.imageUrl}" alt="Nounours ${
        productBought.name
      }"/></div>
                <h2 class="product-name">${productBought.name}</h2>
                <p>${productBought.price / 100}€</p>
                <button id="${
                  productBought.uniqueId
                }" onclick="Controller.deleteBasketLine(this);">Retirer du panier</button>
            </article>`;
    }
    document.getElementById("listProductBought").innerHTML += /*html*/ `
        <article class="basket">
